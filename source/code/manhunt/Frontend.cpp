...
// Do not copy game colours during DLL static initialization. Game memory may
// only be accessed after InitializeASI has validated the executable.
...
void CFrontend::Print16(const wchar_t* text, float x, float y, float sizex, float sizey, float unk, eFontType font)
{
	Call<0x5E5980, const wchar_t*, float, float, float, float, float, eFontType>(text, x, y, sizex, sizey, unk, font);
}
