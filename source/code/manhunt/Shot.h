#pragma once
#include "Weapon.h"
// Inherited order, and measured to be wrong: both the sniper rifle and the
// revolver report 4 while their type data calls them SC_INSTANT. The damage at
// +0x18 of the same structure reads correctly, so the pointer is right and only
// this list is off. Nothing decides anything by it - it is written to the log
// and no further - and it must not be relied on before it is re-derived.
enum eShotClass {
	SC_BASIC,
	SC_INSTANT,
